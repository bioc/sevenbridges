Volume = setRefClass("Volume", contains = "Item",
                     ...
                     methods = list(
                         ...
                         update = function(description = NULL,
                                           service = NULL, ...) {
                         ...
                         delete = function() {
                             auth$api(path = paste0("storage/volumes/", id),
                                      method = "DELETE")
                         },

                         import = function(location  = NULL,
                                           project   = NULL,
                                           name      = NULL,
                                           overwrite = FALSE, ...) {
                             ...
                             res = auth$api(path = "storage/imports",
                         ...
                         export = function(file     = NULL,
                             ...
                             res = auth$api(path = "storage/exports",
                                            body = body, method = "POST")
                         ...
                         get_import_job = function(job_id = NULL) {
                             ...
                             res = auth$api(path = paste0("storage/imports/", job_id),
                                            body = body, method = "GET")

                             # insert
                             if (length(import_job)) {
                                 idx = which(job_id == sapply(import_job,
                                                              function(x) x$id))
                                 if (length(idx)) {
                                     import_job[[idx]] <<- res
                                 } else {
                         ...
                         get_export_job = function(job_id = NULL) {
                             ...
                             res = auth$api(path = paste0("storage/exports/", job_id),
                                            body = body, method = "GET")
                             # insert
                             if (length(export_job)) {
                                 idx = which(job_id == sapply(export_job,
                                                              function(x) x$id))
                                 if (length(idx)) {
                                     export_job[[idx]] <<- res
                                 } else {
